...
21. Navigate to the Shared Components of the app, select **Lists**, then open **LLM Conversations - Top**.
...
22. Edit the following at the end sequence and select **Create**.
...
23. Open Page 13, **Multicloud Overview**, in Page Designer to verify the list entry.

24. Confirm the imported page includes a region named **WingmateChat** in the page body.

25. Select the **WingmateChat** region and confirm the region **Static ID** is `wingmate-chat`.

26. Confirm the **startWingmate** button exists in the **WingmateChat** region.

27. Confirm the imported **Chat** dynamic action is attached to the **startWingmate** button.

28. Confirm the **Show AI Assistant** true action exists. You will connect this action to `wingmate_multicloud_rag` after creating the AI Configuration.

29. Confirm the **Hide** action targets the **startWingmate** button, then save the page.

30. Create Resource Analytics adapter views for the Multicloud page.
...
31. Create and validate the synthetic multicloud graph.
...
32. Navigate to **Shared Components**, then **AI Configurations**.
...
33. Create an AI Configuration with these values:
...
34. Add SQL-based RAG Sources to `wingmate_multicloud_rag`:
...
35. Navigate to SQL Commands in the SQL Workshop and validate the RAG source SQL as `WINGMATE`.
...
36. Configure the imported AI Assistant action by navigating to **Show AI Assistant** in the navigation tree under the Multicloud chat region: **startWingmate** -> **Chat** -> **Show AI Assistant**.
...
38. Create a **Classic Report** region for the Multicloud inventory summary.

    ![Identity Table](./images/update-identity.png "")

39. Name the report **MultiCloud Insights** and set the **SQL Query** to the following:
