...
You will inspect ALH transformation options and the pipeline evidence created in Lab 1, then run short validation queries. You will not start a long ingestion or medallion rebuild. The staged setup recorded selected SQL, Data Transforms, and database-job outcomes in `SEER_GOLD` audit tables so you can review the complete pipeline consistently; those audit tables are workshop assets, not built-in Oracle dictionary views.
...
4. Return to the SQL worksheet and review the latest ALH pipeline executions recorded during Lab 1:
